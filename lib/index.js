...
function nocache(req){
	return req.query.hasOwnProperty("nocache");
}
...
CEX.prototype.putAndServe = function(data,req,res){
	this.put(data,req);
	this.send(data,res, nocache(req));
};

CEX.prototype.send = function(data, res, nocache){
	if(!res.get('Content-Type')){
		res.header('Content-Type','application/json');
	}
	if(!nocache && this.maxAge > 0 ){
